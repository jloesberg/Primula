...
growth.1 <- lmer(log.ros.areaT1 ~ log.ros.area +(1|plot), data = clip, REML = F)
growth.2 <- lmer(log.ros.areaT1 ~ log.ros.area*trt +(1|plot), data = clip, REML= F)
growth.3 <- lmer(log.ros.areaT1 ~ log.ros.area + trt + log.total.seeds + (1|plot), data = clip, REML= F)
growth.4 <- lmer(log.ros.areaT1 ~ log.ros.area + trt + log.total.seeds + trt:log.total.seeds + log.ros.area:trt + (1|plot), data = clip, REML= F)
growth.5 <- lmer(log.ros.areaT1 ~ log.ros.area + log.total.seeds + trt + log.total.seeds:trt + log.ros.area:log.total.seeds + log.ros.area:trt + log.ros.area:log.total.seeds:trt + (1|plot), data = clip, REML= F)

growth.6 <- lmer(log.ros.areaT1 ~ log.ros.area + trt + log.cap + (1|plot), data = clip, REML= F)
growth.7 <- lmer(log.ros.areaT1 ~ log.ros.area + trt + log.cap + trt:log.cap + log.ros.area:trt + (1|plot), data = clip, REML= F)
growth.8 <- lmer(log.ros.areaT1 ~ log.ros.area + log.cap + trt + log.cap:trt + log.ros.area:log.cap + log.ros.area:trt + log.ros.area:log.total.seeds:trt + (1|plot), data = clip, REML= F)

#add in SM data for fun
growth.10 <- lmer(log.ros.areaT1 ~ log.ros.area*sm_avg +(1|plot), data = clip, REML= F)
growth.11 <- lmer(log.ros.areaT1 ~ log.ros.area + sm_avg + log.total.seeds + (1|plot), data = clip, REML= F)
growth.12 <- lmer(log.ros.areaT1 ~ log.ros.area + sm_avg + log.total.seeds + sm_avg:log.total.seeds + log.ros.area:sm_avg + (1|plot), data = clip, REML= F)
growth.13 <- lmer(log.ros.areaT1 ~ log.ros.area + log.total.seeds + sm_avg + log.total.seeds:sm_avg + log.ros.area:log.total.seeds + log.ros.area:sm_avg + log.ros.area:log.total.seeds:sm_avg + (1|plot), data = clip, REML= F)

growth.14 <- lmer(log.ros.areaT1 ~ log.ros.area + sm_avg + log.cap + (1|plot), data = clip, REML= F)
growth.15 <- lmer(log.ros.areaT1 ~ log.ros.area + sm_avg + log.cap + sm_avg:log.cap + log.ros.area:sm_avg + (1|plot), data = clip, REML= F)
growth.16 <- lmer(log.ros.areaT1 ~ log.ros.area + log.cap + sm_avg + log.cap:sm_avg + log.ros.area:log.cap + log.ros.area:sm_avg + log.ros.area:log.total.seeds:sm_avg + (1|plot), data = clip, REML= F)

gr.mods <- mget(ls(pattern = "growth")) #make a list of all of those models that start with gm (^ means start with)
gr_AICc<-aictab(cand.set = gr.mods, modnames = NULL,second.ord=TRUE,nobs=NULL,sort=TRUE)

summary(growth.11) #hmm SM is a better variable. Seems odd but ok
#rm(list = ls()[grepl("growth", ls())]) #removes these models
```

```{r growth figures - sm no int}
...
pred.output <- predict(growth.11, newdata = pred.dat, type="response", re.form=~0) #re.form = ~0 tells it to not include random effects
...
```
```{r growth figures 2 - sm with int}
#create df with all possible x values
#this is for model without an interaction
pred.dat <-  expand.grid(log.ros.area = seq(2, 5.5, by = .1), sm_avg = seq(.1, .25, by = .01), log.total.seeds = seq(0, 5.3, by = .1))
pred.output <- predict(growth.12, newdata = pred.dat, type="response", re.form=~0) #re.form = ~0 tells it to not include random effects
pred.output <- as.data.frame(pred.output)
pred.dat.output <- cbind(pred.dat, pred.output) %>% 
  rename(log.ros.areaT1 = pred.output)
...
ggplot(clip, aes(x = log.ros.area, y = log.ros.areaT1))+
  geom_point()+
  geom_line(data = subset(pred.dat.output, trt == "irrigated" & log.total.seeds == 5.3), aes(x = log.ros.area, y = log.ros.areaT1), color = "blue")+
  geom_line(data = subset(pred.dat.output,  trt == "control" & log.total.seeds == 5.3), aes(x = log.ros.area, y = log.ros.areaT1), color = "grey", size = 1)+
  geom_line(data = subset(pred.dat.output, trt == "drought" & log.total.seeds == 5.3), aes(x = log.ros.area, y = log.ros.areaT1), color = "red", size = 1)+
  geom_line(data = subset(pred.dat.output,  trt == "irrigated" & log.total.seeds == 0), aes(x = log.ros.area, y = log.ros.areaT1),  linetype = "dashed", color = "blue")+
  geom_line(data = subset(pred.dat.output, trt == "control" & log.total.seeds == 0), aes(x = log.ros.area, y = log.ros.areaT1), color = "grey", linetype = "dashed", size = 1)+
  geom_line(data = subset(pred.dat.output, trt == "drought" & log.total.seeds == 0), aes(x = log.ros.area, y = log.ros.areaT1), color = "red", linetype = "dashed", size = 1)
...
pf.1 <- glmer(pflowerT1 ~ log.ros.area +(1|plot), data = clip, family = "binomial")
pf.2 <- glmer(pflowerT1 ~ log.ros.area*trt +(1|plot), data = clip, family = "binomial")
pf.3 <- glmer(pflowerT1 ~ log.ros.area + trt + log.total.seeds + (1|plot), data = clip, family = "binomial")
pf.4 <- glmer(pflowerT1 ~ log.ros.area + trt + log.total.seeds + trt:log.total.seeds + log.ros.area:trt + (1|plot), data = clip, family = "binomial")
pf.5 <- glmer(pflowerT1 ~ log.ros.area + log.total.seeds + trt + log.total.seeds:trt + log.ros.area:log.total.seeds + log.ros.area:trt + log.ros.area:log.total.seeds:trt + (1|plot), data = clip, family = "binomial")

pf.6 <- glmer(pflowerT1 ~ log.ros.area + trt + log.cap + (1|plot), data = clip, family = "binomial")
pf.7 <- glmer(pflowerT1 ~ log.ros.area + trt + log.cap + trt:log.cap + log.ros.area:trt + (1|plot), data = clip, family = "binomial")
pf.8 <- glmer(pflowerT1 ~ log.ros.area + log.cap + trt + log.cap:trt + log.ros.area:log.cap + log.ros.area:trt + log.ros.area:log.total.seeds:trt + (1|plot), data = clip, family = "binomial")

pf.10 <- glmer(pflowerT1 ~ log.ros.area*sm_avg +(1|plot), data = clip, family = "binomial")
pf.11 <- glmer(pflowerT1 ~ log.ros.area + sm_avg + log.total.seeds + (1|plot), data = clip, family = "binomial")
pf.12 <- glmer(pflowerT1 ~ log.ros.area + sm_avg + log.total.seeds + sm_avg:log.total.seeds + log.ros.area:sm_avg + (1|plot), data = clip, family = "binomial")
pf.13 <- glmer(pflowerT1 ~ log.ros.area + log.total.seeds + sm_avg + log.total.seeds:sm_avg + log.ros.area:log.total.seeds + log.ros.area:sm_avg + log.ros.area:log.total.seeds:sm_avg + (1|plot), data = clip, family = "binomial")

pf.14 <- glmer(pflowerT1 ~ log.ros.area + sm_avg + log.cap + (1|plot), data = clip, family = "binomial")
pf.15 <- glmer(pflowerT1 ~ log.ros.area + sm_avg + log.cap + sm_avg:log.cap + log.ros.area:sm_avg + (1|plot), data = clip, family = "binomial")
pf.16 <- glmer(pflowerT1 ~ log.ros.area + log.cap + sm_avg + log.cap:sm_avg + log.ros.area:log.cap + log.ros.area:sm_avg + log.ros.area:log.total.seeds:sm_avg + (1|plot), data = clip, family = "binomial")
...
#this is for model without an interaction
pred.dat <-  expand.grid(log.ros.area = seq(2, 5.5, by = .1), trt = c("irrigated", "control", "drought"), log.total.seeds = seq(0, 5.3, by = .1))
pred.output <- predict(pf.3, newdata = pred.dat, type="response", re.form=~0) #re.form = ~0 tells it to not include random effects
pred.output <- as.data.frame(pred.output)
pred.dat.output <- cbind(pred.dat, pred.output) %>% 
  rename(log.ros.areaT1 = pred.output)
...
```{r poisson}
nof.1 <- glmer(no.flowersT1 ~ log.ros.area + (1|plot), data = clip, family = "poisson")
nof.2 <- glmer(no.flowersT1 ~ log.ros.area*trt + (1|plot), data = clip, family = "poisson")
nof.3 <- glmer(no.flowersT1 ~ log.ros.area + trt + log.total.seeds + (1|plot), data = clip, family = "poisson")
nof.4 <- glmer(no.flowersT1 ~ log.ros.area + trt + log.total.seeds + trt:log.total.seeds + log.ros.area:trt + (1|plot), data = clip, family = "poisson")
nof.5 <- glmer(no.flowersT1 ~ log.ros.area + log.total.seeds + trt + log.total.seeds:trt + log.ros.area:log.total.seeds + log.ros.area:trt + log.ros.area:log.total.seeds:trt + (1|plot), data = clip, family = "poisson")

nof.6 <- glmer(no.flowersT1 ~ log.ros.area + trt + log.cap + (1|plot), data = clip, family = "poisson")
nof.7 <- glmer(no.flowersT1 ~ log.ros.area + trt + log.cap + trt:log.cap + log.ros.area:trt + (1|plot), data = clip, family = "poisson")
nof.8 <- glmer(no.flowersT1 ~ log.ros.area + log.cap + trt + log.cap:trt + log.ros.area:log.cap + log.ros.area:trt + log.ros.area:log.total.seeds:trt + (1|plot), data = clip, family = "poisson")

nof.10 <- glmer(no.flowersT1 ~ log.ros.area*sm_avg + (1|plot), data = clip, family = "poisson")
nof.11 <- glmer(no.flowersT1 ~ log.ros.area + sm_avg + log.total.seeds + (1|plot), data = clip, family = "poisson")
nof.12 <- glmer(no.flowersT1 ~ log.ros.area + sm_avg + log.total.seeds + sm_avg:log.total.seeds + log.ros.area:sm_avg + (1|plot), data = clip, family = "poisson")
nof.13 <- glmer(no.flowersT1 ~ log.ros.area + log.total.seeds + sm_avg + log.total.seeds:sm_avg + log.ros.area:log.total.seeds + log.ros.area:sm_avg + log.ros.area:log.total.seeds:sm_avg + (1|plot), data = clip, family = "poisson")

nof.14 <- glmer(no.flowersT1 ~ log.ros.area + sm_avg + log.cap + (1|plot), data = clip, family = "poisson")
nof.15 <- glmer(no.flowersT1 ~ log.ros.area + sm_avg + log.cap + sm_avg:log.cap + log.ros.area:sm_avg + (1|plot), data = clip, family = "poisson")
nof.16 <- glmer(no.flowersT1 ~ log.ros.area + log.cap + sm_avg + log.cap:sm_avg + log.ros.area:log.cap + log.ros.area:sm_avg + log.ros.area:log.total.seeds:sm_avg + (1|plot), data = clip, family = "poisson")

flowers <- mget(ls(pattern = "nof.")) #make a list of all of those models that start with gm (^ means start with)
flowers_AICc<-aictab(cand.set = flowers, modnames = NULL,second.ord=TRUE,nobs=NULL,sort=TRUE)
...
#this is for model without an interaction
pred.dat <-  expand.grid(log.ros.area = seq(2, 5.5, by = .1), sm_avg = seq(.1, .25, by = .01), log.total.seeds = seq(0, 5.3, by = .1))
pred.output <- predict(nof.11, newdata = pred.dat, type="response", re.form=~0) #re.form = ~0 tells it to not include random effects
pred.output <- as.data.frame(pred.output)
pred.dat.output <- cbind(pred.dat, pred.output) %>% 
  rename(log.ros.areaT1 = pred.output)
...
```{r survival}
sur.1 <- glmer(psurvivalT1 ~ log.ros.area +(1|plot), data = clip, family = "binomial")
sur.2 <- glmer(psurvivalT1 ~ log.ros.area*trt +(1|plot), data = clip, family = "binomial")
sur.3 <- glmer(psurvivalT1 ~ log.ros.area + trt + log.total.seeds + (1|plot), data = clip, family = "binomial")
sur.4 <- glmer(psurvivalT1 ~ log.ros.area + trt + log.total.seeds + trt:log.total.seeds + log.ros.area:trt + (1|plot), data = clip, family = "binomial")
sur.5 <- glmer(psurvivalT1 ~ log.ros.area + log.total.seeds + trt + log.total.seeds:trt + log.ros.area:log.total.seeds + log.ros.area:trt + log.ros.area:log.total.seeds:trt + (1|plot), data = clip, family = "binomial")

sur.6 <- glmer(psurvivalT1 ~ log.ros.area + trt + log.cap + (1|plot), data = clip, family = "binomial")
s.7 <- glmer(psurvivalT1 ~ log.ros.area + trt + log.cap + trt:log.cap + log.ros.area:trt + (1|plot), data = clip, family = "binomial")
sur.8 <- glmer(psurvivalT1 ~ log.ros.area + log.cap + trt + log.cap:trt + log.ros.area:log.cap + log.ros.area:trt + log.ros.area:log.total.seeds:trt + (1|plot), data = clip, family = "binomial")

sur.10 <- glmer(psurvivalT1 ~ log.ros.area*sm_avg +(1|plot), data = clip, family = "binomial")
sur.11 <- glmer(psurvivalT1 ~ log.ros.area + sm_avg + log.total.seeds + (1|plot), data = clip, family = "binomial")
sur.12 <- glmer(psurvivalT1 ~ log.ros.area + sm_avg + log.total.seeds + sm_avg:log.total.seeds + log.ros.area:sm_avg + (1|plot), data = clip, family = "binomial")
sur.13 <- glmer(psurvivalT1 ~ log.ros.area + log.total.seeds + sm_avg + log.total.seeds:sm_avg + log.ros.area:log.total.seeds + log.ros.area:sm_avg + log.ros.area:log.total.seeds:sm_avg + (1|plot), data = clip, family = "binomial")

sur.14 <- glmer(psurvivalT1 ~ log.ros.area + sm_avg + log.cap + (1|plot), data = clip, family = "binomial")
s.15 <- glmer(psurvivalT1 ~ log.ros.area + sm_avg + log.cap + sm_avg:log.cap + log.ros.area:sm_avg + (1|plot), data = clip, family = "binomial")
sur.16 <- glmer(psurvivalT1 ~ log.ros.area + log.cap + sm_avg + log.cap:sm_avg + log.ros.area:log.cap + log.ros.area:sm_avg + log.ros.area:log.total.seeds:sm_avg + (1|plot), data = clip, family = "binomial")
...
pred.output <- predict(sur.1, newdata = pred.dat, type="response", re.form=~0) #re.form = ~0 tells it to not include random effects
pred.output <- as.data.frame(pred.output)
pred.dat.output <- cbind(pred.dat, pred.output) %>% 
  rename(log.ros.areaT1 = pred.output)
...
ggplot(clip, aes(x = log.ros.area, y = psurvivalT1))+
  geom_point()+
  geom_line(data = pred.dat.output, aes(x = log.ros.area, y = log.ros.areaT1))
  geom_line(data = subset(pred.dat.output,  sm_avg == .13 & log.total.seeds == 5.3), aes(x = log.ros.area, y = log.ros.areaT1), color = "grey", size = 1)+
  geom_line(data = subset(pred.dat.output, sm_avg == .1 & log.total.seeds == 5.3), aes(x = log.ros.area, y = log.ros.areaT1), color = "red", size = 1)+
  geom_line(data = subset(pred.dat.output,  sm_avg == .25 & log.total.seeds == 0), aes(x = log.ros.area, y = log.ros.areaT1),  linetype = "dashed", color = "blue")+
  geom_line(data = subset(pred.dat.output, sm_avg == .13 & log.total.seeds == 0), aes(x = log.ros.area, y = log.ros.areaT1), color = "grey", linetype = "dashed", size = 1)+
...
#this is for model without an interaction
pred.dat <-  expand.grid(log.ros.area = seq(2, 5.5, by = .1), sm_avg = seq(.1, .25, by = .01), log.total.seeds = seq(0, 5.3, by = .1))
pred.output <- predict(sur.11, newdata = pred.dat, type="response", re.form=~0) #re.form = ~0 tells it to not include random effects
pred.output <- as.data.frame(pred.output)
pred.dat.output <- cbind(pred.dat, pred.output) %>% 
  rename(log.ros.areaT1 = pred.output)
...
```{r}
#summary table of n, etc

n2021 <- join %>% 
  group_by(year, plot, trt, treatment) %>% 
  summarize(n = n())
#write_csv(n2021, "../tables/clipping.sample.size.csv")
...
noseeds<- glmer(total.seeds ~ 1 + (1|plot), data = subset(join, pseeds ==1), family = "poisson")
noseeds1 <- glmer(as.numeric(total.seeds) ~ log.ros.area + (1|plot), data = subset(join, pseeds == 1), family = "poisson")
...
noseeds4 <- glmer(total.seeds ~ log.ros.area + treatment + (1|plot), data = subset(join, pseeds == 1), family = "poisson")
anova(noseeds1, noseeds4 ,test="Chisq") #LTR test
summary(noseeds4)

noseeds5 <- glmer(total.seeds ~ log.ros.area + trt+treatment + (1|plot), data = subset(join, pseeds == 1), family = "poisson")
...
ggplot(subset(join, pseeds == 1), aes(x=log.ros.area, y=total.seeds))+
  ...
  geom_line(data = pred.drought, aes(x = log.ros.area, y = pred.drought), color = Z[5], linewidth = 1.5) +
  geom_line(data = pred.irrigated, aes(x = log.ros.area, y = pred.irrigated), color = Z[1], linewidth = 1.5) + 
  geom_line(data = pred.control, aes(x = log.ros.area, y = pred.control), color = Z[3], linewidth = 1.5) + 
  ...
  #scale_x_continuous(expand = c(0, 0), limits = c(0, 6))
ggsave("./Figures/clipping experiment/poisson.seedproduction.png", width = 4, height = 4)
