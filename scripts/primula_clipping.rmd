...
(growth2 <- ggplot(clip, aes(x = log.ros.area, y = log.ros.areaT1))+
  geom_abline(slope=1, intercept = 0, color= "black", linewidth = .2)+
  geom_point(aes(size = log.total.seeds), shape = 1, color = "black", alpha = 0.65)+
  #scale_color_gradientn(colours = rainbow(5))+
  #geom_point(aes(color = log.total.seeds), size = .00000001)+

  geom_line(data = subset(pred.dat.output, log.total.seeds == 5), aes(x = log.ros.area, y = log.ros.areaT1, color = log.total.seeds), linewidth = 1.5)+
  #geom_line(data = subset(pred.dat.output, log.total.seeds == 4), aes(x = log.ros.area, y = log.ros.areaT1), color = "darkorchid3")+
  geom_line(data = subset(pred.dat.output, log.total.seeds == 2.5), aes(x = log.ros.area, y = log.ros.areaT1, color = log.total.seeds), linetype = "longdash", linewidth = 1.5)+
 # geom_line(data = subset(pred.dat.output, log.total.seeds == 2), aes(x = log.ros.area, y = log.ros.areaT1), color = "darkorchid1")+
  #geom_line(data = subset(pred.dat.output,  log.total.seeds == 1), aes(x = log.ros.area, y = log.ros.areaT1, color = log.total.seeds), linetype = "dashed", linewidth = 1.5)+
  geom_line(data = subset(pred.dat.output, log.total.seeds == 0), aes(x = log.ros.area, y = log.ros.areaT1, color = log.total.seeds), linetype = "dashed", linewidth = 1.5)+
  scale_color_gradient2(midpoint = 2.5, low = "orchid1", mid = "darkorchid2",
                            high = "#5e1d7e", space = "Lab", guide=guide_colourbar(reverse = TRUE) )+
  scale_y_continuous(expand = c(0, 0), limits = c(2, 6))+
  scale_x_continuous(expand = c(0, 0), limits = c(2, 6))+
  labs(x = "log(Rosette Area) in 2021", y = "log(Rosette Area) in 2022", size = "", color = "log(Seeds)\nin 2021")+
  theme(axis.line = element_line(colour = 'black', linewidth = 1.1), 
        text = element_text(size = 20),
        legend.title = element_text(size = 10),
        legend.text = element_text(size = 8))+
...
growth2
ggsave(plot = growth2, file = "C:/Users/Jenna/OneDrive - The University Of British Columbia/Data Projects/Primula/Figures/clipping experiment/3growth result fig.png", width =6, height =5, dpi = 300)
```
