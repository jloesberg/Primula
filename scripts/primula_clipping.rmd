...
quant <- clip %>% 
  #group_by(trt) %>% 
  summarize(perc10 = quantile(log.total.seeds, probs = 0.1, na.rm = T),
            perc90 = quantile(log.total.seeds, probs = 0.9, na.rm = T))
#create df with all possible x values
pred.dat <-  expand.grid(log.ros.area = seq(min(clip$log.ros.area), 5.15, by = .1), log.total.seeds = c(0,3.71))
...
(growth <- ggplot(clip, aes(x = log.ros.area, y = log.ros.areaT1))+
  ...
  geom_line(data = subset(pred.dat.output.g, log.total.seeds == 3.71), aes(x = log.ros.area, y = log.ros.areaT1),color = "darkorchid4", linewidth = 1.25)+
