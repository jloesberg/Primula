...
# subsetting - get rid of plots 12 and 14 (only 3 observations per plot)
...
ggplot(clip, aes(x = log.ros.area, y = log.ros.areaT1))+
  ...
  facet_wrap(~plot)
ggplot(clip, aes(x = log.ros.area, y = as.numeric(pflowerT1)-1))+
  geom_point()+
...
(no.seeds.boxplot.trt <- 
  ...
  labs(y = "Total seeds", x = "Number of flowers clipped")+
   theme(legend.position="none")+
    scale_y_continuous(breaks=log(c(0.25, 1,2,5,10,20,100)), labels=c(0,0,2,5, 10,20, 100)))
#ggsave(plot = no.seeds.boxplot.trt, file = "C:/Users/Jenna/OneDrive - The University Of British Columbia/Data Projects/Primula/Figures/clipping experiment/round 2/no.seeds.boxplot.trt.png", width = 5, height = 4, dpi = 300)
...
no.seeds.boxplot
#ggsave(plot = no.seeds.boxplot, file = "C:/Users/Jenna/OneDrive - The University Of British Columbia/Data Projects/Primula/Figures/clipping experiment/round 2/no.seeds.boxplot.png", width = 5, height = 5, dpi = 300)
```
...
(remember not to use REML estimates!)
...
we used likelihood ratio tests (LRTs) as an additional method to quantify the difference between models with or without traits. A significant χ2 value (p< 0.05) from an LRT indicated that including trait values significantly changes model fit.
...
The significance of each fixed effect in the best-fit model was tested using likelihood ratio tests on nested models.
...
(growth <- ggplot(clip, aes(x = log.ros.area, y = log.ros.areaT1))+
  ...
  geom_point(aes(size = log.total.seeds), shape = 1, color = "black")+
  #scale_color_gradientn(colours = rainbow(5))+
  #geom_point(aes(color = log.total.seeds), size = .00000001)+
  #scale_color_gradient2(midpoint = 2.5, low = "orchid1", mid = "darkorchid2", high = "darkorchid4", space = "Lab" )+
  ...
  #geom_line(data = subset(pred.dat.output.g, log.total.seeds == 4), aes(x = log.ros.area, y = log.ros.areaT1), color = "darkorchid3")+
  geom_line(data = subset(pred.dat.output.g, log.total.seeds == 3), aes(x = log.ros.area, y = log.ros.areaT1), color = "darkorchid2", linetype = "longdash", linewidth = 1.5)+
 # geom_line(data = subset(pred.dat.output.g, log.total.seeds == 2), aes(x = log.ros.area, y = log.ros.areaT1), color = "darkorchid1")+
 ...
 # geom_line(data = subset(pred.dat.output.g, log.total.seeds == 0), aes(x = log.ros.area, y = log.ros.areaT1), color = "grey")+
  scale_y_continuous(expand = c(0, 0), limits = c(2, 6))+
  scale_x_continuous(expand = c(0, 0), limits = c(2, 6))+
  labs(x = "log(Rosette Area) in 2021", y = "log(Rosette Area) in 2022", color = "log(Seeds) in 2021", size = "log(Seeds)\nin 2021")+
...
(growth2 <- ggplot(clip, aes(x = log.ros.area, y = log.ros.areaT1))+
  geom_abline(slope=1, intercept = 0, color= "black", linewidth = .2)+
  geom_point(aes(size = total.seeds), shape = 1, color = "black", alpha = 0.75)+
  scale_size(range=c(1.25, 2.5))+
  geom_line(data = subset(pred.dat.output.g, log.total.seeds ==5), aes(x = log.ros.area, y = log.ros.areaT1, color = (log.total.seeds)), linewidth = 1.5)+
  #geom_line(data = subset(pred.dat.output.g, log.total.seeds == 4), aes(x = log.ros.area, y = log.ros.areaT1), color = "darkorchid3")+
  geom_line(data = subset(pred.dat.output.g, log.total.seeds == 2.5), aes(x = log.ros.area, y = log.ros.areaT1, color = (log.total.seeds)), linetype = "longdash", linewidth = 1.5)+
  geom_line(data = subset(pred.dat.output.g, log.total.seeds == 0), aes(x = log.ros.area, y = log.ros.areaT1, color = (log.total.seeds)), linetype = "dashed", linewidth = 1.5)+
  scale_color_gradient2(midpoint =2.5, low = "orchid1", mid = "darkorchid2",
                            high = "#5e1d7e", guide = guide_colourbar(reverse = TRUE), limits = c(0, 5.02), breaks = c(0, log(2), log(5), log(10), log(20), log(40), log(80), log(150)), labels = c(0, 2, 5, 10,20, 40, 80, 150))+
  scale_y_continuous(expand = c(0, 0), limits = c(2, 6))+
  scale_x_continuous(expand = c(0, 0), limits = c(2, 6))+
...
(growth3 <- ggplot(clip, aes(x = log.ros.area, y = log.ros.areaT1))+
  geom_abline(slope=1, intercept = 0, color= "black", linewidth = .2)+
  geom_point(aes(size = total.seeds), shape = 1, color = "black", alpha = 0.75)+
  scale_size(range=c(1.25, 2.5))+
  geom_line(data = subset(pred.dat.output.g, log.total.seeds == 5), aes(x = log.ros.area, y = log.ros.areaT1),color = "darkorchid4", linewidth = 1.5)+
 ...
 # geom_line(data = subset(pred.dat.output.g, log.total.seeds == 2), aes(x = log.ros.area, y = log.ros.areaT1), color = "darkorchid1")+
  geom_line(data = subset(pred.dat.output.g,  log.total.seeds == 0), aes(x = log.ros.area, y = log.ros.areaT1), color = "orchid1", linetype = "dashed", linewidth = 1.5)+
  #scale_color_gradient2(midpoint =2.5, low = "orchid1", mid = "darkorchid2", high = "#5e1d7e", guide = guide_colourbar(reverse = TRUE), limits = c(0, 5.02), breaks = c(0, log(2), log(5), log(10), log(20), log(40), log(80), log(150)), labels = c(0, 2, 5, 10,20, 40, 80, 150))+
...
growth2
ggsave(plot = growth3, file = "C:/Users/Jenna/OneDrive - The University Of British Columbia/Data Projects/Primula/Figures/clipping experiment/7growth result fig.png", width =6, height =5, dpi = 300)
```
...
anova(pflower_no.trt, pflower_sizealone, test="LRT")
...
## These are the AIC model selection anlayses - not what we ended up going with but leaving for now.
...
no.seeds_LRT = data.frame(var = NA,
                        chi = NA, 
                        pvalue = NA)
#colnames(no.flowers_LRT) <- c("var", "chi", "p-value")
...
a1 <- anova(noseeds, noseeds1 ,test="Chisq") #LTR test
no.seeds_LRT[1,] <- c("size", a1[2,6], a1[2,8])

#yep its better

noseeds2 <- glmer(total.seeds ~ log.ros.area + trt + (1|plot), data = subset(clip, pseeds == 1), family = "poisson")
a2 <- anova(noseeds1, noseeds2 ,test="Chisq") #LTR test
no.seeds_LRT[2,] <- c("treatment", a2[2,6], a2[2,8])
#yep

noseeds3 <- glmer(total.seeds ~ log.ros.area*trt + (1|plot), data = subset(clip, pseeds == 1), family = "poisson")
a3 <- anova(noseeds2, noseeds3 ,test="Chisq") #LTR test
no.seeds_LRT[3,] <- c("size x treatment", a3[2,6], a3[2,8])
...
write.csv(x = no.seeds_LRT, file = "C:/Users/Jenna/OneDrive - The University Of British Columbia/Data Projects/Primula/tables/clipping experiment/round 3/seeds_LRT.csv")
...
no.seeds <- 
  ggplot(subset(clip, pseeds == 1), aes(x=log.ros.area, y=total.seeds, color = trt))+
  ...
  theme(axis.line = element_line(colour = 'black', linewidth = 1.1), text = element_text(size = 15))+ 
    ...
    theme(legend.position="none")
no.seeds
#ggsave(plot = no.seeds, file = "C:/Users/Jenna/OneDrive - The University Of British Columbia/Data Projects/Primula/Figures/clipping experiment/round 2/poisson.seedproduction.png", width = 5, height = 5, dpi = 300)
