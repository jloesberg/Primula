...
- To do: use sarah's code with the townsendia solution to having multiple growth functions feed into one IPM big marrix!
first, just write out the params to have 6 rows (for each combination of reproduction and trt! )

```{r}
#this is code from Sarah that has one function!! 
# 
...
params=data.frame(
	growth.int = c(NA,NA, NA, NA, NA, NA),
...
params$reproduction <- c("flowered", "veg", "flowered", "veg", "flowered", "veg")

#growth intercepts - different for each treatment
params$growth.int[params$trt == "control"] <- fixef(gm17)[1] + fixef(gm17)[5]*av.grow.max.temp
...
params$pflow.slope[params$reproduction == "veg"] <- fixef(fm.f16)[2] # probability of flowering slope for veg plants
params$pflow.slope[params$reproduction == "flowered"] <- fixef(fm.f16)[2] + fixef(fm.f16)[5] # probability of flowering slope for flowered plants
...
params_list <-  filter(params) %>% split(., seq(nrow(.))) #this splits up params into 6 rows, which I can feed into the function below
...
bigmatrix<-function(params_list, mat.size)
  ...
  out_lamb <- data.frame(lambda = lambda(IPMmat), trt = params_list$trt, reproduction = params_list$reproduction)
