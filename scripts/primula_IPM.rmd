...
- I think Tom's code can help me figure out how to not have seperate functions for each treatment (ie use some form of ifelse)
...
I've just added in the mean temp, so these are specific to one specific temperature (in the future, I think I can write a loop to get a range of temperatures?)
...
#GROWTH FROM SIZE X TO XP for CONTROL...xp is y
...
g.yx.irr<-function(y,x,params){
  #xb=pmin(pmax(x,minRealsize),maxRealsize) #Transforms all values below/above limits in min/max size (floor & ceiling)
  #growth_to_newsize <-params$growth.int.irr + params$growth.slope*xb
  #growth_sd <- params$growth.sd
  #return(dnorm(y,mean=growth_to_newsize,sd=growth_sd))
  dnorm(y,mean=params$growth.int.irr + params$growth.slope*x,sd=params$growth.sd)

}

g.yx <- function(y,x,params,trt){
  irrigated=ifelse()
...
P.control <- function(y, x, params){
  s.x(x, params)*g.yx.control(y, x, params)
}
...
# #PRODUCTION OF seedlings by X-SIZED MOMS that flowered the previous year
...
f.yx.veg=function(x,params) { 		
	return(pf.x.veg(x,params)*exp(params$no.flow.int+params$no.flow.slope*x)*
    params$p.flower.makes.seed*params$no.seeds*params$prob.seed.germ) #not sure what this should be
}



#SIZE DISTRIBUTION OF RECRUITS - separate function (before was part of f.yx). Now it feeds into a stage so cant be feeding back into the kernel
...
bigmatrix<-function(params
  ...
  Fmat[1,3:(n+2)]<-f.yx.flowered(y,params) #
  ...
  Tmat[3:(n+2),2]<- prob.1leaf.to.2leaf *recruit.size(y,params) * h
  
  # Growth/survival transitions among cts sizes
  Tmat[3:(n+2),3:(n+2)]<-
    t(outer(y,y,P.control,params=params)) * h 
...
primula_lambda <- bigmatrix(params = params)
