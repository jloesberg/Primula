...
#write.csv(tot.seeds, "C:/Users/Jenna/Dropbox/Williams' Lab/Cowichan IDE/Cowichan_DemographyData/Dodecatheon/Dodecatheon_Seed_Counts.csv", row.names = F)
tot.seeds <- tot.seeds %>% 
  mutate(plot= as.character(plot),
         year = as.character(year))
...
```{r}
# (3) how many flowers get to make seeds - successful fruitset - can this be a constant or does it vary across treatments, year, etc
test <- left_join(Dodecatheon, seedset)
test <- test %>% filter(!is.na(seed.set))
hist(test$seed.set)

ggplot(test, aes(x = log.ros.area, y = seed.set))+
  geom_point()+
  #geom_smooth(method="glm", method.args=list(family="poisson"), se = F)+
  facet_wrap(~trt)

ggplot(test, aes(x = trt, y = seed.set))+
  geom_boxplot()
#what am i trying to ask here - does fruitset differ between treatments?

seed.set.trt <- test %>% 
  group_by(trt, year) %>% 
  summarize(mean = mean(seed.set),
            sd = sd(seed.set))
#so seed set definitely varies by year and treatment. And is surprisingly low!
