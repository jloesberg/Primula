...
#choose this one for all climate varibles minus annual
climate <- climate_scale %>% 
  filter(year != "2023") %>%
  select(-c(starts_with("annual")))
clim.names <- colnames(climate)
year <- climate[,1]
...
corrplot(M, method="circle",type="upper",insig = "p-value", sig.level=.1)
#so yes, some are correlated! onwards
#note - jenn wants p-vaues here!
remove(M)
```
...
```
PC1 had the strongest loadings for winter temperature and growing season temperature
PC2 had strongest loadings for summer temperature, growing season precipitation, fall temperature, and winter precipitation
```{r}
loadings.save <- pca_loadings %>% 
  arrange(desc(abs(PC1)))
write.csv(x = loadings.save, file = "C:/Users/Jenna/OneDrive - The University Of British Columbia/Data Projects/Primula/figures/climate/PCA long list/loadings table.csv")
```
